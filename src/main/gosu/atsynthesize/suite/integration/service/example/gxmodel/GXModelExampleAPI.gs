...
class GXModelExampleAPI 
  ...
  // Query operator stub (provided by Guidewire platform at runtime)
  static var equals : String = "Equals"
  ...
  protected function processExampleGxModelGetAllUsersWsiServiceNoInputArrayOutput() 
    ...
    if (usrs.Count > 0) {
      ...
      usrs.eachWithIndex(\ u, i -> {
                          var gxUser = new User(u as entity.User)
                          result[i] = gxUser
      })
    }
